...
// Exported (not just used internally) so tests can assert the full
// mapping table directly, same rationale as lib/db.js's exported
// wrapWithAutoMigrate.
...
// Factory shape so tests can swap in an in-memory fake sql + fake Stripe
// verify/client without a live DB or Stripe connection — same opts.getSql
// seam lib/crudHandler.js established (see api/booking-requests.js /
// api/cron-reminders.js for the same pattern applied to other handlers).
export function createStripeWebhookHandler(opts = {}) {
  const getSql = opts.getSql || db;
  const verifyWebhook = opts.verifyWebhook || verifyStripeWebhook;
  const getStripeClient = opts.getStripeClient || stripeClient;
  ...
  return async function handler(request) {
    ...
    let event;
    try {
      event = await verifyWebhook(rawBody, signature, webhookSecret);
    } catch (err) {
