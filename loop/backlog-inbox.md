...
<task_item>
  ...
  <status>READY_FOR_PM</status>
  <priority>HIGH</priority>
  <title>Demo mode: trainer/realestate/insurance personas show ฿0 on the Home hero after seeding</title>
  <description>Discovered while running the full test battery for TSK-025/026 (not from a screenshot): `tests/check-demo-data.js` fails 3 of 22 assertions -- "[trainer] Home hero amount is non-zero after seeding, got: ฿0", same for realestate and insurance. Confirmed via `git stash` that this reproduces identically on the commit before TSK-025/026 (pre-existing, not caused by this epoch's changes) -- likely fallout from TSK-023 (Fee/Tip/Expense/Sessions removed from the Add-session form; revenue now attributed at the moment it's known: from the paid invoice, from the package's apportioned purchase price at delivery, or from the new one-field Cash gate -- see squad-handshake-engineer.md's TSK-023 writeup). Demo-data seeding (`demo/index.html` and/or wherever per-persona demo jobs/invoices/packages are generated) was apparently never updated to route through one of those three revenue paths for the trainer/realestate/insurance personas, so their seeded jobs carry no revenue signal any of the new code paths pick up, and the Home hero -- the app's headline "how much have I earned" number, the very first thing a prospective user sees on `/demo` -- shows ฿0. This directly undercuts the demo's purpose (showing the app looks used/valuable).</description>
  <researcher_notes>Not yet root-caused past "which test fails" -- next step is finding exactly how demo-data seeding creates its jobs/packages/invoices per persona and which of the three TSK-023 revenue paths (invoice.youReceive on markJobPaid, package purchase-price apportionment on delivery, or the one-field Cash gate) it should be routed through, then confirming the other 3 personas (whichever check-demo-data.js already reports as passing) show the right shape so the fix is consistent across all personas, not just a 3-persona patch. Marked HIGH: `/demo` is a first-impression, user-facing surface, and this has likely been silently broken since TSK-023 shipped (2026-07-26).</researcher_notes>
</task_item>
