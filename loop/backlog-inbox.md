...
<task_item>
  <id>TSK-029</id>
  <source>RESEARCHER_SQUAD</source>
  <status>READY_FOR_PM</status><!-- SHIPPED by Engineer-Squad 2026-08-26: commit de4e7b9, PR #100 (open, P1 label applied), subscribed for CI/review activity. Debounced onClientSearchInput() (150ms), pre-indexed jobs/packages by clientId in reload() so activePackageFor/clientPackages/clientStage/clientProspectService are O(1) lookups instead of O(jobs)/O(packages) filters per client per render, and deduped computeClientsNeedingAttention()'s concurrent renderHome()+renderCustomers() calls within one reload() via a self-clearing in-flight promise (deliberately NOT cached across separate calls -- invoices have no in-memory global and can be mutated by paths that don't call reload(), confirmed by a real regression caught locally in check-home-today-v2.js before this shipped, see squad-handshake-engineer.md). New Playwright coverage (check-clients-stage.js §6) asserts the render is debounced -- one call per keystroke burst, not one per character. Full battery clean: 18 Node + 34 Playwright suites, 0 failures, 0 console errors. -->
  <priority>HIGH</priority>
...
<task_item>
  <id>TSK-030</id>
  <source>RESEARCHER_SQUAD</source>
  <status>READY_FOR_PM</status><!-- SHIPPED by Engineer-Squad 2026-08-26: commit c915d1c, PR #102 (open). dbAll/dbGet/dbDel/dbGetByUsername now wire req.onerror, matching dbPut/dbAdd's existing pattern; new tests/check-db-error-handling.js proves each rejects instead of hanging. Full battery clean: 18 Node + 37 Playwright suites, 0 failures. See squad-handshake-engineer.md. -->
  <priority>HIGH</priority>
...
<task_item>
  <id>TSK-031</id>
  <source>RESEARCHER_SQUAD</source>
  <status>READY_FOR_PM</status><!-- SHIPPED by Engineer-Squad 2026-08-27: PR #104 (open, branched from origin/main, merged origin/main in once TSK-030/PR #102 landed mid-cycle). All 6 modal open/close function pairs now store/restore focus, move initial focus into the modal, and share one delegated Escape/Tab-trap keydown listener. New tests/check-modal-focus.js. Full battery clean: 18 Node + 36 Playwright suites, 0 failures. See squad-handshake-engineer.md. -->
  <priority>HIGH</priority>
