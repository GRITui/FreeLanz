...
<task_item>
  <id>TSK-018</id>
  <source>OWNER_POPUP</source>
  <status>NEEDS_OWNER_REVIEW</status><!-- Part (1) SHIPPED by Engineer-Squad 2026-07-25: commit cf3eb73, PR #72 (merged). renderPipelineTimeline() (app/app.js:4645) now also sources dated points from job.due (+ its linked booking via job.dueBookingCuid), not just job.subTasks -- a job.due backed by a linked booking renders as a dot (matches an 'exact' step); one without renders as a bar (matches a 'by' step). New check-scheduling.js section 6a (jobB's gate-set job.due renders as one timeline dot mid-flow). Full battery clean, check-scheduling.js 66/66. Part (2) -- removing job.subTasks entirely -- remains NEEDS_OWNER_REVIEW, unstarted, blocked on the open design question below. See squad-handshake-engineer.md. -->
  <priority>MEDIUM</priority>
